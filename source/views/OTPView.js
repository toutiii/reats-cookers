...
import { ActivityIndicator, Platform, Text, StyleSheet, TextInput, View } from "react-native";
...
export default function OTPView({ ...props }) {
    ...
    async function saveTokenData() {
        ...
        await SecureStore.setItemAsync("refreshToken", `${tokenData.token.refresh}`);
        await SecureStore.setItemAsync("accessToken", `Bearer ${tokenData.token.access}`);
        await SecureStore.setItemAsync("phoneNumber", `${props.route.params.item.phone}`);
    }
            ...
            <View>
                ...
                {!props.route.params.auth && isOTPValidationRequestSuccessful && (
                    <CustomAlert
                        ...
                        onConfirmPressed={() => {
                            setShowAlert(false);
                            if (!props.route.params.auth && isOTPValidationRequestSuccessful) {
            ...
            <View style={styles.container} key={randomNumber}>
                {OTPValue.map((digit, index) => (
                    <TextInput
                        key={index}
                        style={styles.box}
                        autoComplete={Platform.OS === "android" && index === 0
                            ? "sms-otp"
                            : "off"}
                        textContentType={
