...
const DocumentsScreen = () => {
  ...
  return (
    <ThemedView className="flex-1">
        ...
        <LinearGradient colors={["#FFAF70", "#FF5963"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
          <View className="px-6 pt-24 pb-12">
            ...
            <Text className="text-white text-base font-normal">
              Quelques étapes pour compléter votre profil{"\n"}
              et commencer à proposer vos services culinaires
            </Text>
          </View>
        </LinearGradient>
      </View>
      ...
      <View className="px-6 pt-6">
        <Heading className="mb-4">Documents complétés</Heading>

        <VStack space="xl">
          <DocumentMenuItem title="Informations personnelles" completed={true} />
        </VStack>
      </View>

      <Box className="px-6 mt-12">
        <Button size="xl" onPress={() => navigation.navigate("InformationVerificationScreen")}>
          <ButtonText className="text-lg font-medium">Soumettre mon dossier</ButtonText>
        </Button>
      </Box>
    </ThemedView>
  );
