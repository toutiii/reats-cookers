...
import { ActivityIndicator, Animated, KeyboardAvoidingView, ScrollView, View } from "react-native";
...
export default function Form({ ...props }) {
    ...
    const [
        newItemCopy,
        setNewImteCopy, // eslint-disable-line no-unused-vars
    ] = useState(props.item);
    ...
    const [
        validationErrors,
        setValidationErrors
    ] = useState(getInitialErrorsState(fieldKeys));
    ...
    const removeItemAction = async () => {
        ...
        try {
            const userID = await getItemFromSecureStore("userID");
            const accessToken = await getItemFromSecureStore("accessToken");
            const result = await props.action(newItem, props.url, "DELETE", userID, accessToken);
                ...
                <View style={styles_form.container}>
                    ...
                    {showAlertDeleteAccount && (
                        <CustomAlert
                            ...
                            showCancelButton={true}
                            cancelButtonColor='red'
                            confirmText={all_constants.custom_alert.homeview.keep_account}
                            cancelText={all_constants.custom_alert.homeview.delete_account}
                            onConfirmPressed={() => {
